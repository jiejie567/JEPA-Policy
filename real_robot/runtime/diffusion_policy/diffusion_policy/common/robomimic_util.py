...
def set_env_meta_controller_input_type(env_meta, input_type):
    """Set the arm-controller input mode across Robosuite API versions."""
    if input_type not in ("absolute", "delta"):
        raise ValueError(f"Unsupported controller input type: {input_type!r}")
    ...
    # Robosuite <= 1.4 used a flat controller configuration.
    controller_config["input_type"] = input_type
    controller_config["control_delta"] = input_type == "delta"


def get_robot_arm_controller(robot):
    """Return the realized arm controller across Robosuite API versions."""
    legacy_controller = getattr(robot, "controller", None)
    ...
    arm_controllers = [
        controller
        for controller in part_controllers.values()
        if hasattr(controller, "goal_pos") and hasattr(controller, "goal_ori")
    ]
    if len(arm_controllers) != 1:
        raise RuntimeError(
            "Expected exactly one realized arm controller, found "
            f"{len(arm_controllers)}"
        )
    return arm_controllers[0]
...
class RobomimicAbsoluteActionConverter:
    def __init__(self, dataset_path, algo_name='bc'):
        ...
        set_env_meta_controller_input_type(abs_env_meta, "absolute")
        ...
        for robot in abs_env.env.robots:
            controller = get_robot_arm_controller(robot)
            input_type = getattr(controller, "input_type", None)
            use_delta = getattr(controller, "use_delta", None)
            assert input_type == "absolute" or use_delta is False
