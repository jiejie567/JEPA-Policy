...
class AsyncVectorEnv(VectorEnv):
    ...
    def reset_async(self, seed=None, options=None):
        # Gym 0.26+ forwards seed and options from VectorEnv.reset. This
        # implementation retains the legacy explicit seed() workflow used by
        # Diffusion Policy, but must accept the newer keyword arguments.
        if seed is not None or options is not None:
            raise NotImplementedError(
                "Pass seeds through AsyncVectorEnv.seed() before reset(); "
                "reset options are not supported by this compatibility env.")
        self._assert_is_running()
        if self._state != AsyncState.DEFAULT:
            raise AlreadyPendingCallError(
