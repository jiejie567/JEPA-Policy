...
class TrainingAgent:
    ...
    def update(
        ...
        if self.sigreg is not None and self.config.optimization.sigreg_weight > 0:
            ...
            # Reuse the exact encoder tensor already shared by the action and
            # future losses. This keeps the ablation to one changed loss term
            # and avoids an extra stochastic/expensive encoder forward.
            obs_emb = cached_obs_emb
            if obs_emb is None:
                obs_emb = self.encoder(obs, None)
            if obs_emb.dim() == 2:
