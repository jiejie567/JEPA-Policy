...
from jepa_policy.config import NetworkConfig, TaskConfig
...
from jepa_policy.encoders_dino import FrozenDINOv2ObsEncoder
from jepa_policy.networks.chitfm_dino_aux import ChiTransformerDINOAux
...
def get_network(
    ...
    from jepa_policy.networks.chiunet import ChiUNet
    from jepa_policy.networks.jannerunet import JannerUNet
    from jepa_policy.networks.mlp import MLP, VanillaMLP
    from jepa_policy.networks.rnn import RNN, VanillaRNN
    from jepa_policy.networks.sudeepdit import SudeepDiT
    ...
    network_class = {
        ...
        "sudeepdit": SudeepDiT,
        "chitransformer_dino_aux": ChiTransformerDINOAux,
    ...
    if network_config.network_type == "mlp":
        ...
        return network_class(
            **common_params,
            ...
            n_cond_layers=network_config.n_cond_layers,
            timestep_emb_type=network_config.timestep_emb_type,
            dino_out_dim=network_config.dino_out_dim,
        )
...
def get_encoder(network_config: NetworkConfig, task_config: TaskConfig):
    if task_config.obs_type == "image":
        encoder_type = getattr(network_config, "encoder_type", "image")
    elif task_config.obs_type in ["state", "keypoint"]:
        encoder_type = "mlp"
    else:
        raise ValueError(f"Invalid observation type: {task_config.obs_type}")
    ...
    if encoder_type == "identity":
        ...
        kwargs = {
            "shape_meta": task_config.shape_meta,
            "dino_model_name": network_config.dino_model_name,
            "dino_embed_dim": network_config.dino_embed_dim,
            "emb_dim": network_config.emb_dim,
            "use_seq": network_config.use_seq,
            "keep_horizon_dims": network_config.keep_horizon_dims,
            "resize_shape": task_config.resize_shape,
