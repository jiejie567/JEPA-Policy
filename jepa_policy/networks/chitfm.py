...
from jepa_policy.embeddings import SUPPORTED_TIMESTEP_EMBEDDING
from jepa_policy.networks.base import BaseNetwork
...
class ChiTransformer(BaseNetwork):
    ...
    def _forward_features(
        ...
        return y, scalar_output, decoder_output_norm
    ...
    def forward_with_aux(
        ...
        y, scalar, decoder_output = self._forward_features(
            x, s, t, condition, future_input=future_input
        )
        if self.n_future_tokens <= 0 or self.future_head is None:
            raise RuntimeError("This ChiTransformer was not configured with future tokens")
